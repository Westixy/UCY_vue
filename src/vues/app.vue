    ...
    <v-layout row wrap :style="{height:'100vh',overflow:'auto'}">
      <v-flex sm4 class="flex-collumn">
        <div class="centerit triple">
          <v-btn 
            v-on:click.left="playSound('entreeJoueur')"
            v-on:contextmenu="selectFile('entreeJoueur')"
            class="btn-simple yellow darken-2"
            primary
          >
            <v-icon v-if="paths.entreeJoueur===null">do_not_disturb_on</v-icon>
            Entrée joueurs
          </v-btn>
          <v-btn
            v-on:click.left="playSound('starting6')"
            v-on:contextmenu="selectFile('starting6')"
            class="btn-simple yellow darken-2"
            primary
          >
            <v-icon v-if="paths.starting6===null">do_not_disturb_on</v-icon>
            Starting 6
          </v-btn>
          <v-btn
            v-on:click.left="playSound('intro')"
            v-on:contextmenu="selectFile('intro')"
            class="btn-simple blue-grey darken-4"
            primary
          >
            <v-icon v-if="paths.intro===null">do_not_disturb_on</v-icon>
            Intro
          </v-btn>
        </div>
        <div class="centerit">
          <v-btn 
            v-on:click.left="playSound('goalTeam1')"
            v-on:contextmenu="selectFile('goalTeam1')"
            class="btn-simple green darken-1"
            primary
          >
            <v-icon v-if="paths.goalTeam1===null">do_not_disturb_on</v-icon>
            Goal team 1
          </v-btn>
        </div>
        <div class="centerit">
          <v-btn
            v-on:click.left="playSound('pub')"
            v-on:contextmenu="selectFile('pub')"
            class="btn-simple deep-orange darken-2"
            primary
          >
            <v-icon v-if="paths.pub===null">do_not_disturb_on</v-icon>
            PUB
          </v-btn>
        </div>
      </v-flex>
      <v-flex sm4 class="flex-collumn">
        ...
        <div class="centerit">
          ...
          <v-btn
            v-on:click.left="playMult('arretsDeJeu')"
            v-on:contextmenu="selectFolder('arretsDeJeu')"
            class="btn-aj"
            primary
          >
            <div>
              <v-icon v-if="paths.arretsDeJeu===null">do_not_disturb_on</v-icon>
              Arrêts de jeu
              <br>
              <v-btn
                :disabled="mult.arretsDeJeu.sounds.length===0"
                fab
                :dark="isDark"
                small
                v-on:click.stop="nextSound(mult.arretsDeJeu)"
              >
          ...
          <v-text-field
            label="Now"
            readonly
            :value="path.basename(mult.arretsDeJeu.actual===null?'':mult.arretsDeJeu.actual)"
            class="actual"
          />
          <br>
          <v-text-field 
            label="Next"
            readonly
            :value="path.basename(mult.arretsDeJeu.next===null?'':mult.arretsDeJeu.next)"
            class="next"
          />
        </div>
        <div class="centerit">
          <v-btn 
            v-on:click.left="playTimeout()"
            v-on:contextmenu="selectFile('timeOut')"
            class="btn-simple purple darken-2"
            primary
          >
            <v-icon v-if="paths.timeOut===null">do_not_disturb_on</v-icon>
            Time out
      ...
      <v-flex sm4 class="flex-collumn">
        <div class="centerit">
          <v-btn 
            v-on:click.left="playSound('penality')"
            v-on:contextmenu="selectFile('penality')"
            class="btn-simple red darken-2" 
            primary
          >
            <v-icon v-if="paths.penality===null" >do_not_disturb_on</v-icon>
            Penality
          </v-btn>
          <v-btn 
            class="btn-simple pink darken-1" 
            primary  
            v-on:click.left="playSoundSpe()"
            v-on:contextmenu="selectFile('minutes2')"
          >
            <v-icon v-if="paths.minutes2===null" >do_not_disturb_on</v-icon>
            2 Minutes 
            <v-btn 
              fab 
              :dark="isDark"
              small
              v-on:click.stop="resetSoundSpe()"
            >
        ...
        <div class="centerit">
          <v-btn 
            v-on:click.left="playSound('goalTeam2')" 
            v-on:contextmenu="selectFile('goalTeam2')" 
            class="btn-simple green darken-1" 
            primary 
          >
            <v-icon v-if="paths.goalTeam2===null">do_not_disturb_on</v-icon>
            Goal team 2
          </v-btn>
        </div>
        <div class="centerit">
          <v-btn 
            v-on:click.left="playMult('ambiance')" 
            v-on:contextmenu="selectFolder('ambiance')"
            class="btn-simple lime darken-2" 
            primary
          >
            <v-icon v-if="paths.ambiance===null">do_not_disturb_on</v-icon>
            Ambiance
          </v-btn>
        </div>
      </v-flex>
...
export default {
  ...
  methods: {
          ...
          fs.readFile(this.configFile, (err, data) => {
            if (err) {
              ...
              try {
                ...
                if (obj.restartAfterStop !== undefined)
                  this.mult.restartAfterStop = obj.restartAfterStop
      ...
      if (mult === undefined) {
        ...
        if (mult.sounds.length === 0) {
          ...
          for (const file of files) {
            ...
            if (['mp3', 'ogg', 'wav'].indexOf(ext.toLowerCase()) === -1)
              continue
            mult.sounds.push(path.join(this.paths[what], file))
          }
          if (mult.sounds.length === 0)
            this.showToast("Folder doesn't have any music")
        }
        this.multSetTracks(mult)
        if (mult.actual === null) return this.music.stop()
        if (this.$refs.audio.src === encodeSrc(mult.actual) && playing === true)
          this.nextSound(mult)
        this.playSound(undefined, {
          onEnd: () => this.nextSound(mult),
          defpath: mult.actual,
          restart: this.mult.restartAfterStop,
        })
      }
    },
    prevSound(mult) {
      mult.selected =
        mult.selected > 0 ? mult.selected - 1 : mult.sounds.length - 1
      this.multSetTracks(mult)
      if (mult.actual === null) return this.music.stop()
    ...
    nextSound(mult) {
      const playing = this.music.playing
      mult.selected =
        mult.selected < mult.sounds.length - 1 ? mult.selected + 1 : 0
      this.multSetTracks(mult)
      if (mult.actual === null) return this.music.stop()
      this.music.switch(mult.actual)
    ...
    multSetTracks(mult) {
      ...
      mult.next =
        mult.sounds.length > mult.selected + 1
          ? mult.sounds[mult.selected + 1]
          : mult.sounds[0]
    },
  },
  mounted() {
